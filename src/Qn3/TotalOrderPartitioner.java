...
public class TotalOrderPartitioner<K extends WritableComparable<?>,V>
    extends Partitioner<CompositeKey3, LongWritable> implements Configurable {
...
//      Class<K> keyClass = (Class<K>)job.getMapOutputKeyClass();
      ...
      RawComparator<K> comparator =
    	        (RawComparator<K>) WritableComparator.get(LongWritable.class);
  ...
  // change to own data type
  public int getPartition(CompositeKey3 key, LongWritable value, int numPartitions) {
	  
	// pass key.getArticle_ID() instead of key
    return partitions.findPartition(new LongWritable(key.getArticle_ID()));
  }
